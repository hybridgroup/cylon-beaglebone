...
Beaglebone.prototype._pwmWrite = function(pinNum, value, period, duty, eventName) {
  ...
  if (pin != null) {
    ...
    pin.on('pwmWrite', function(val) {
      return _this.connection.emit(eventName, value);
    });
...
Beaglebone.prototype.pwmWrite = function(pinNum, value, freq) {
  pwm = this._pwmPeriodAndDuty(value, freq, pulseWidth);

  this._pwmWrite(pinNum, value, pwm.period, pwm.duty, 'pwmWrite');
...
Beaglebone.prototype.servoWrite = function(pinNum, value, freq, pulseWidth) {
  ...
  this._pwmWrite(pinNum, value, pwm.period, pwm.duty, 'servoWrite');
...
Beaglebone.prototype._pwmPeriodAndDuty = function(value, freq, pulseWidth) {
  ...
  if (pulseWidth != null) {
    maxDuty = pulseWidth.max - pulseWidth.min;
    scaledDuty = (~(value & 0xff)).fromScale(0, 180);
    duty = Math.round(pulseWidth.min + (maxDuty * scaledDuty));
  } else {
