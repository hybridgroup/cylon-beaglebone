  ...
  if (pin.connected) {
    ...
    pin.on('error', function(err) {
      if ((err.count < 10) && (err.code == 'ENOENT') && (err.errno == 34)) {
        ...
        _this.connection.emit('err', value);
      }
    });
...
Beaglebone.prototype.pwmWrite = function(pinNum, value, freq) {
  var scaledDuty = (value).fromScale(0, 255), pwm;

  freq = freq || this.DEFAULT_FREQ;
  pwm = Cylon.IO.Utils.periodAndDuty(scaledDuty, freq, null, 'low');

  this._pwmWrite(pinNum, value, pwm.period, pwm.duty, 'pwm');
};

Beaglebone.prototype.servoWrite = function(pinNum, value, freq, pulseWidth) {
  var scaledDuty = (value).fromScale(0, 180), pwm;

  freq = freq || this.DEFAULT_FREQ;
  pwm = Cylon.IO.Utils.periodAndDuty(scaledDuty, freq, pulseWidth, 'low');
