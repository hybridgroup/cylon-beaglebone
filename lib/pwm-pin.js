...
PwmPin.prototype.pwmWrite = function(duty) {
  ...
  this.writePeriod(function(){
    var __this = _this;

    _this.writeDuty(duty, function() {
      __this.emit('pwmWrite', duty);
    });
  });
};

PwmPin.prototype.writePeriod = function(callback) {
  var _this = this;
  ...
  fs.appendFile(_this._periodPath(), _this.period, function(err) {
    if (err) {
     _this.emit('error', err);
      ...
      _this.emit('writePeriod', _this.period);
...
PwmPin.prototype.writeDuty = function(duty, callback) {
  var _this = this;
  this.duty = duty;
